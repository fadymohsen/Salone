    ...
    <div className="min-h-[calc(100dvh-56px)] flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <h1 className="font-serif text-2xl font-bold text-glam-text mb-1">{t.register.title}</h1>
        ...
        <div className="bg-white rounded-3xl p-7 border border-border shadow-sm shadow-primary/5">
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            <div>
              <label htmlFor="name" className="block text-xs font-semibold text-glam-text mb-1.5">{t.register.fullName} <span className="text-primary" aria-hidden="true">*</span></label>
              <div className="relative">
                ...
                <input id="name" type="text" required autoComplete="name" value={form.name}
                  ...
                  className={`${INPUT} ps-10`} placeholder={t.register.namePlaceholder} />
              </div>
            </div>

            <div>
              <label htmlFor="email" className="block text-xs font-semibold text-glam-text mb-1.5">{t.common.email} <span className="text-primary" aria-hidden="true">*</span></label>
              <div className="relative">
            ...
            <div>
              <label htmlFor="phone" className="block text-xs font-semibold text-glam-text mb-1.5">{t.common.phone} <span className="text-muted font-normal">({t.common.optional})</span></label>
              <div className="relative">
                ...
                <input id="phone" type="tel" autoComplete="tel" value={form.phone}
                  ...
                  className={`${INPUT} ps-10`} placeholder={t.register.phonePlaceholder} />
              </div>
            </div>

            <div>
              <label htmlFor="password" className="block text-xs font-semibold text-glam-text mb-1.5">{t.common.password} <span className="text-primary" aria-hidden="true">*</span></label>
              <div className="relative">
              ...
              <p className="text-xs text-muted mt-1 ps-1">{t.register.passwordHint}</p>
            </div>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <><Loader2 size={16} className="animate-spin" aria-hidden="true" /> {t.register.creating}</> : t.register.createAccount}
            </button>
          </form>

          <p className="mt-5 text-center text-xs text-muted">
            {t.register.alreadyHave}{" "}
            <Link href={l("/account/login")} className="text-primary font-semibold hover:underline cursor-pointer">{t.common.signIn}</Link>
          </p>
        </div>
      </div>
    </div>
