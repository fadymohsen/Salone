...
function BookingFormContent() {
  ...
  const [form, setForm] = useState({ clientName: "", clientPhone: "", clientEmail: "", serviceType: "", bookingDate: "", bookingTime: "" });
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [meRes, svcRes] = await Promise.all([fetch("/api/auth/me"), fetch("/api/services")]);
  ...
  useEffect(() => { const p = searchParams.get("service"); if (p) setForm(prev => ({ ...prev, serviceType: p })); }, [searchParams]);
  useEffect(() => { if (services.length > 0 && !form.serviceType) setForm(prev => ({ ...prev, serviceType: services[0].name })); }, [services]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <div className="max-w-md mx-auto px-6 pt-8">
        ...
        <div className="bg-white rounded-3xl p-6 border border-border shadow-sm shadow-primary/5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div>
              <label htmlFor="serviceType" className="block text-xs font-semibold text-glam-text mb-1.5">{t.booking.service}</label>
              <div className="relative">
                <select id="serviceType" value={form.serviceType} onChange={(e) => setForm({ ...form, serviceType: e.target.value })}
                  disabled={services.length === 0} className={`${INPUT_CLASS} pe-10 cursor-pointer appearance-none disabled:opacity-60 disabled:cursor-not-allowed`}>
                  {services.length === 0 ? (
                ...
                <ChevronDown size={15} className="absolute end-4 top-1/2 -translate-y-1/2 text-muted pointer-events-none" aria-hidden="true" />
              </div>
            </div>
            ...
            <button type="submit" disabled={!form.bookingDate || !form.bookingTime}
