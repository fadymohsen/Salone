...
export default async function AdminDashboard({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const stats = [
    ...
    { label: d.allTime, value: totalCount, sub: d.totalClients, Icon: Users, color: "text-violet-500" },
  ];
    ...
    <div className="px-6 py-8 max-w-3xl mx-auto space-y-8">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-serif text-base font-bold text-glam-text">{d.todaySchedule}</h2>
      ...
      <div>
        <h2 className="font-serif text-base font-bold text-glam-text mb-3">{d.monthlyOverview}</h2>
        <AdminCalendar dayCountMap={dayCountMap} today={today} />
      </div>

      <div>
        <h2 className="font-serif text-base font-bold text-glam-text mb-3">{d.recentlyAdded}</h2>
