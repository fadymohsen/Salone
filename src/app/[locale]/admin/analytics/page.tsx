...
export default async function AnalyticsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  try {
    [allBookings, monthCount] = await Promise.all([
      prisma.booking.findMany({ select: { serviceType: true, status: true, bookingDate: true, clientPhone: true } }),
  ...
  const services = Object.entries(serviceMap).sort((x, y) => y[1] - x[1]);
  ...
  const kpis = [
    { label: a.totalBookings, value: allBookings.length, Icon: CalendarDays },
    { label: a.thisMonth, value: monthCount, Icon: TrendingUp },
    { label: a.uniqueClients, value: uniqueClients, Icon: Users },
  ];

  return (
    <div className="px-6 py-8 max-w-3xl mx-auto space-y-8">
      <div>
        <h1 className="font-serif text-2xl font-bold text-glam-text">{a.title}</h1>
          ...
          {last7.map((d) => (
            <div key={d.date} className="flex-1 flex flex-col items-center gap-1 group">
              ...
              <div className="w-full relative flex items-end" style={{ height: "88px" }}>
                <div className="w-full bg-primary rounded-t-lg transition-all duration-500"
                  style={{ height: `${(d.count / maxDay) * 88}px`, minHeight: d.count ? "6px" : "2px", opacity: d.count ? 1 : 0.12 }} />
              </div>
      ...
      <div className="bg-white rounded-2xl border border-border p-6">
        ...
        <div className="flex flex-wrap gap-3">
          {Object.entries(statusMap).map(([status, count]) => {
            ...
            return (
              <div key={status} className={`flex items-center gap-2.5 px-4 py-3 rounded-xl border ${meta?.color ?? "bg-gray-50 text-gray-500 border-gray-100"}`}>
